...
from stochastic_torus import TorusToGym
...
torus_result_folder = 'results/torus'
mean_rnd = {5:100, 10:700, 20:3200}
...
# <<<<<<<<<<<<<<<<<<<<<<<<<<<< utils >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
def dst_folder(torus_size, period):
  dst = torus_result_folder + '/{0}x{0}'.format(torus_size)
...
if __name__ == '__main__':
  ...
  parser = argparse.ArgumentParser(description='Runs DQN on specified torus envs.')
  parser.add_argument('--torus_size', type=int, default=5, choices=[5,10,20],
                      help='Size of the undlying square grid of a torus')
  ...
  print 'torus size:', args.torus_size
  print 'update period:', args.period
  print 'envs id:', args.envs

  file_template = dst_folder(args.torus_size, args.period)
  file_template = file_template + '/dqn_burtsev_{0}.json'
  dqn_params = burtsev_params(mean_rnd[args.torus_size])
  print 'dst files template:', file_template

  n_runs = 10
  time_per_run = []
  for env_id in args.envs:
    env = TorusToGym.load(args.torus_size, env_id, period=args.period)
    seeds = np.random.randint(0,4000, (n_runs,))
    loggers = []

    print 'Test DQN on torus({0}x{0})#{1}:'.format(args.torus_size, env_id)
